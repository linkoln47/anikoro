...
-- Genre dimension. id is the MAL genre id (stable and globally unique on MAL),
-- supplied by the hydrator rather than generated. MAL's public anime `genres`
-- field is a single flat list mixing genres, themes, and demographics; they are
-- all stored uniformly here.
CREATE TABLE genres (
    id INTEGER PRIMARY KEY,
    name TEXT NOT NULL,
    CHECK (id > 0),
...
-- the genres MAL assigns to it, rebuilt from MAL on each detail hydration the
-- same way anime_relations is.
